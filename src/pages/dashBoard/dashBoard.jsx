...
const Dashboard = ({handleUser}) => {
    ...
    return (
        <div className="dash-board">
            <h1 className="dash-board-main-title"> Personal Dashboard Of {userName}</h1>
            <div className="dash-board-balance-cash"  style={{zIndex:buyOrSell==="buy"?'100':'0'}}>
                <DashBoardAttribute title="Cash Balance" value={Math.floor(userData.balance) + "$"}/>
            </div>
            <div className="dash-board-pl">
                <DashBoardAttribute title="P&L Daily"  value={
            ...
            <div className="dash-board-stocks-value">
                <DashBoardAttribute title="Portfilo Value" value={stocksValue.toFixed(0) +"$"}/>
            </div>
            <div className="dash-board-total-balance">
                <DashBoardAttribute title="Total Balance" value={(userData.balance+stocksValue).toFixed(0)+"$"}/>
            </div>
            ...
            {buyOrSell && <BuyAndSell portfolioStocksList={portfolioStocksList} toSell={sell} toBuy={buy} action={buyOrSell} handleExit={handleBuy} />}
