    ...
    return(
        <form onSubmit={handleSubmit}>
            <div className="img-div">
                <img className="login-img" src={StockWelcomeImage} alt="Stock Market Image"/>
                {<div id="login">
                    <h1>Register! </h1>
                    <p>
                        <label>Name</label>
                        <input type="text" required onBlur={(event) => handleUserInput(
                            'name', event.target.value)}/>
                    </p>
                    <p>
                        <label>Email address</label>
                        <input className={ emailNotValid && "input-argument-not-valid"}
                            type="email"
                            equired
                            onBlur={(event)=>{
                                handleUserInput(
                    ...
                    {emailNotValid && <div className={`field-not-valid ${scaleMessage ? 'blink' : ''}`}> <p>Email not valid</p> </div>}
                    <p>
                    ...
                    <p>
                        ...
                        <input className={ passwordsNoMatch && "input-argument-not-valid"}
                               type="password"
                               onBlur={(event)=>{
                    ...
                    {passwordsNoMatch && <div className={`field-not-valid ${scaleMessage ? 'blink' : ''}`}> <p>Passwords not match</p> </div>}
                    {emailIsAlreadyUsed && <div className="field-not-valid"> <p>This email is already registered. Please use a different email or log in.</p> </div>}

                    <p id="actions">
                        <button type="button"
                                className='LoginOrRegister'
                                onClick={handleSwitch}> Already have an account
                        </button>
